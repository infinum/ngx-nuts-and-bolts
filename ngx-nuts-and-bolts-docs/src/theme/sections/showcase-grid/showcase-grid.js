...
export default function ShowcaseGrid(props) {
	...
	const ctaTitle = privateType ? (
		...
		<span>
			Want to add your <br /> project to the list?
		</span>
	);
	...
	const items = shuffleArray(itemsData).map((item, index) => {
		...
		return <ShowcaseCard key={index} url={link} imageUrl={image} imageAlt={label} title={label} description={desc} />;
	});
	...
	return (
		...
		<Fragment key={isClient ? 1 : 2}>
			...
			<CtaImageButton title={ctaTitle} buttonLabel={ctaSubtitle} buttonUrl={ctaUrl} imageUrl="/img/showcase/cta.svg" />
