...
Please check out `apps/breadcrumbs-example`. You can run the app with `npm run start:breadcrumbs-example`.
...
- The way routes are defined in `apps/breadcrumbs-example/src/app/app.routes.ts`.
- Different resolvers for breadcrumb data in `apps/breadcrumbs-example/src/app/breadcrumb-resolvers`.
- Example of how to render breadcrumb in `apps/breadcrumbs-example/src/app/components/breadcrumbs`.
...
You can see an example of this in demo app's customer details component (`apps/breadcrumbs-example/src/app/pages/customer-details/customer-details.component.ts`) and route definition where `breadcrumbResolverKey` is set to `location` - matching the input name of the component so that resolved data can be read easily.
